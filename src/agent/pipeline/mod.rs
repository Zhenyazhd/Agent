...
use parser::extract_all_tx_params;
...
pub async fn run_transaction_analysis_pipeline(
    ...
    // ── Extract tx params ──────────────────────────────────────
    let (tx_hashes, chain_id) = extract_all_tx_params(user_message)
        .ok_or_else(|| {
            AgentError::Internal(
                "Could not extract tx hashes and chain ID from message".to_string(),
            )
        })?;

    send_step(effective_tx, &mut steps, AgentStep {
        step_type: StepType::Thinking,
        content: format!("Found {} transaction(s) on chain {}", tx_hashes.len(), chain_id),
        tool_name: None,
        tool_input: None,
        tool_output: None,
    }).await;
    ...
    let mut final_reports: Vec<String> = Vec::new();
    ...
    for tx_hash in &tx_hashes {
        ...
        let report = pass3::run_pass3(agent, report_path, effective_tx, &mut total_iterations, &mut steps).await?;
        final_reports.push(report);
    }

    Ok(AgentResponse {
        steps,
        final_answer: final_reports.join("\n\n---\n\n"),
